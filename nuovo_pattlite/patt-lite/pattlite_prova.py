...
best_file = dataset_name + '_hyperparameters_numClasses7_new.txt'
...
TRAIN_MIN_LR = 1e-6
TRAIN_DROPOUT = 0.1
units_initial = int(best_hp['units_initial'])
...
FT_ES_PATIENCE = 20
FT_DROPOUT = 0.4
units_FT = int(best_hp['units'])
FT_DROPOUT = float(best_hp['dropout'])
FT_LR = best_hp['learning_rate']
l2_FT = float(best_hp['l2'])
...
pre_classification = tf.keras.Sequential([
    tf.keras.layers.Dense(units_initial, activation='relu', kernel_regularizer = l2(l2_initial)),
...
global_average_layer = tf.keras.layers.GlobalAveragePooling2D(name='gap')
pre_classification = tf.keras.Sequential([
    tf.keras.layers.Dense(units_FT, activation='relu', kernel_regularizer = l2(l2_FT)),
    tf.keras.layers.BatchNormalization()
], name='pre_classification')
