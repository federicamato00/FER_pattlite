...
best_file = 'nuovo_pattlite/CK+_hyperparameters_numClasses7/CK+_hyperparameters_numClasses7_new.txt'
...
with open(best_file, 'r') as f:
    ...
    for line in f:
        ...
        if name in ['units']:
            best_hp[name] = int(float(value))  # Converti esplicitamente a int
        else:
...
TRAIN_MIN_LR = 1e-6
TRAIN_DROPOUT = 0.1
TRAIN_DROPOUT = float(best_hp['dropout_rate'])
TRAIN_LR = float(best_hp['learning_rate'])
FT_EPOCH = 500
FT_LR = 1e-5
...
units = int(best_hp['num_units'])
l2_reg = float(best_hp['l2_reg'])
...
pre_classification = tf.keras.Sequential([
    tf.keras.layers.Dense(units, activation='relu', kernel_regularizer=l2(l2_reg)),
...
patch_extraction = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(256, kernel_size=1, strides=1, padding='valid', activation='relu', kernel_regularizer=l2(l2_FT))
], name='patch_extraction')

global_average_layer = tf.keras.layers.GlobalAveragePooling2D(name='gap')
pre_classification = tf.keras.Sequential([
    tf.keras.layers.Dense(32, activation='relu', kernel_regularizer = l2(l2_FT)),
    tf.keras.layers.BatchNormalization()
], name='pre_classification')
...
x = SqueezeLayer(axis=-1)(x)  # Rimuovi la dimensione di sequenza dopo l'attenzione
x = tf.keras.layers.Dropout(FT_DROPOUT)(x)
x = tf.keras.layers.Dense(units_initial, activation='relu')(x)
outputs = prediction_layer(x)
