...
best_file = dataset_name.lower() + '_hyperparameters_new.txt'
...
save_path = 'processed_ckplus_new.h5'
# Carica le immagini
...
patch_extraction = tf.keras.Sequential([
     ...
     tf.keras.layers.BatchNormalization(),
     tf.keras.layers.Dropout(best_hps['dropout_rate_FT']),  # Aggiungi dropout
     tf.keras.layers.Conv2D(256, kernel_size=1, strides=1, padding='valid', activation='relu', kernel_regularizer=l2(best_hps['l2_reg_FT']))
], name='patch_extraction')
