...
BATCH_SIZE_FT = 8
FT_ES_PATIENCE = 20 #numero di epoche di tolleranza per l'arresto anticipato, aumentato da 20 a 40
...
save_path = 'ckplus_data_augmentation.h5'
# Carica le immagini
path_easy = os.path.join('datasets','data_augmentation', dataset_name + '_numClasses7',save_path )
...
patch_extraction = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(256, kernel_size=1, strides=1, padding='valid', activation='relu', kernel_regularizer=l2(best_hps['l2_reg_1']))
], name='patch_extraction')
...
patch_extraction = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(256, kernel_size=1, strides=1, padding='valid', activation='relu', kernel_regularizer=l2(best_hps['l2_reg_FT_1']))
], name='patch_extraction')

global_average_layer = tf.keras.layers.GlobalAveragePooling2D(name='gap')
pre_classification = tf.keras.Sequential([
    tf.keras.layers.Dense(int(best_hps['units_ft']), activation='relu', kernel_regularizer=l2(best_hps['l2_reg_FT_2'])),
    tf.keras.layers.BatchNormalization(),
